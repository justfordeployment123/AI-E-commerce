...
const CATEGORY_META: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  memory:       { label: "Memory",       icon: Cpu,        color: "bg-purple-500/10 text-purple-500" },
  storage:      { label: "Storage",      icon: HardDrive,  color: "bg-zinc-500/10 text-zinc-500" },
  smartwatches: { label: "Smartwatches", icon: Watch,      color: "bg-cyan-500/10 text-cyan-500" },
};
...
export default function CatalogOthersPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const otherItems = items.filter(d => !MAIN_CAT_SLUGS.includes(categorySlug(d).toLowerCase()));
      const otherBcs = bcs.filter(bc => !MAIN_CAT_SLUGS.includes(bc.category.slug.toLowerCase()));
      setDevices(otherItems);
      setBrandCategories(otherBcs);
    } catch { /* ignore */ }
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link href="/catalog" className="text-xs text-zinc-400 hover:text-black flex items-center gap-1 font-bold">
              Device Catalog
            </Link>
            <span className="text-zinc-300">/</span>
            <span className="text-xs text-zinc-600 font-bold">Others</span>
          </div>
          <h1 className="text-2xl font-bold text-zinc-900 mt-1">Other Device Catalog</h1>
          <p className="text-sm text-zinc-400 font-medium mt-1">Manage other accessory and subcategory devices.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 overflow-hidden">
        <div className={`grid ${SCRAPER_ENABLED ? "grid-cols-[2fr_4fr_2fr_2fr_auto_auto]" : "grid-cols-[2fr_6fr_2fr_auto_auto]"} text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-6 py-3 border-b border-zinc-100 bg-zinc-50`}>
          <span>Brand</span><span>Model</span><span>Category</span>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <motion.div key={device.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    className={`grid ${SCRAPER_ENABLED ? "grid-cols-[2fr_4fr_2fr_2fr_auto_auto]" : "grid-cols-[2fr_6fr_2fr_auto_auto]"} items-center px-6 py-4 hover:bg-zinc-50/60 transition-colors cursor-pointer`}>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={form.model}
                    ...
                    placeholder="e.g. DDR4 Desktop RAM"
