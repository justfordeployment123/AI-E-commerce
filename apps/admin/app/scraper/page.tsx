...
import { scraperApi, type ScrapedPriceRow, type ScraperStats } from "../../lib/api";
import { Play, RefreshCw, Search, TrendingUp, CheckCircle2, AlertCircle, Clock } from "lucide-react";
...
export default function ScraperPage() {
  ...
  async function handleRunScraper() {
    ...
    try {
      ...
      // Refresh stats after 3s to show updated counts
      setTimeout(() => { loadStats(); loadPrices(); }, 3000);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Scraped from CeX, Back Market, and Music Magpie. Used as primary source for trade-in pricing.
          </p>
        </div>
        <button
      ...
      {/* Run message */}
      {runMsg && (
        <div className={`flex items-center gap-3 rounded-2xl px-5 py-3 mb-6 text-sm font-bold ${
          ...
          {runMsg}
          {!runMsg.toLowerCase().includes("fail") && <span className="font-normal text-emerald-600 ml-1">— prices will update in the background. Refresh in a few minutes.</span>}
        </div>
      )}
      ...
      <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
        ...
        {loadingTable ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-zinc-400">Click "Run Scraper" to fetch competitor prices for all active devices.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-100">
                    {["Device", "Storage", "CeX Sell", "CeX Cash", "CeX Exchange", "Back Market", "Music Magpie", "Market Price", "Scraped"].map(h => (
                    ...
                    <tr key={row.id} className="hover:bg-zinc-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5">
                        <span className="flex items-center gap-1.5 text-xs text-zinc-400 whitespace-nowrap">
                          ...
                          {new Date(row.scrapedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "2-digit" })}
                        </span>
                      </td>
                    </tr>
                  ))}
